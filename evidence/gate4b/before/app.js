...
(function () {
  ...
  const MODULES = [
    ...
    },
    {
      ...
      description:
        "Handoff distillation ledger: snapshots and open questions. No runtime is attached.",
      noRuntime: true,
    },
  ];

  const DISTILLERY_NOTE =
    "No runtime, entry point, or UI exists for Sovereign Distillery. Startup test not applicable.";
  ...
  const STATE_META = {
    ...
    EXTERNAL: { cls: "badge-ok", label: "External" },
  ...
  function interpretPreflight(value, isPort) {
    ...
    if (typeof value === "object") {
      ...
      if (status) {
        ...
        const badStates = ["down", "missing", "unavailable", "busy", "error", "fail", "failed", "in use"];
  ...
  function applyModuleState(id, record) {
    ...
    if (mod && mod.noRuntime) state = "NOT_STARTED"; // spec 7: distillery never runs
  ...
  async function onAction(id, action) {
    ...
    if (action === "open") {
      ...
      if (rec._url) window.open(rec._url, "_blank", "noopener");
  ...
  async function loadDistillery() {
    ...
    refs.extra.snapshot.textContent =
      firstString(data.snapshot_id, data.snapshot, data.snapshotId) || "—";

    const q =
      data.open_questions !== undefined
        ? data.open_questions
        : data.openQuestions !== undefined
          ? data.openQuestions
          : data.questions;
    if (typeof q === "number") refs.extra.questions.textContent = String(q);
    else if (Array.isArray(q)) refs.extra.questions.textContent = String(q.length);
    else refs.extra.questions.textContent = "—";

    const links = Array.isArray(data.links)
      ? data.links
      : Array.isArray(data.handoff_docs)
        ? data.handoff_docs
