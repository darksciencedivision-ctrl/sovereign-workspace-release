...
const { retrieveAndDeliver } = require("./control/conductor-view");
...
async function handleOperatorText(payload) {
  ...
  let res;
  try {
    res = await retrieveAndDeliver({ message: text,
    ...
    if (res.context && res.context.notice) pushTranscriptTurn({ utc: new Date().toISOString(),
      dir: "sys", text: res.context.notice });
  } catch (e) {
...
async function runObjective(objective, options = {}) {
  ...
  const selected = selectObjectiveRecipients(liveWorkerRecords(), registered);
  try {
