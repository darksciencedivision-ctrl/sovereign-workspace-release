...
const { composeOwnDigest, paneNumber, SOURCE } = require("./workspace-journal");
const { characterBudget } = require("./conductor-view");
...
async function delegateToPane(io, options = {}) {
  ...
  const rawPrompt = options.paneId && options.task ? flattenBody(delegationPrompt(task)) : "";
  ...
  await record({ status: "delegation_requested", prompt: rawPrompt });
  ...
  await record({ status: result.answered ? "answered" : result.refused ? "write_refused"
    ...
    prompt: result.delivered ? rawPrompt : "",
...
module.exports = {
  ...
  delegationPrompt, delegateToPane, selectObjectiveRecipients, paneNumber, SOURCE,
};
