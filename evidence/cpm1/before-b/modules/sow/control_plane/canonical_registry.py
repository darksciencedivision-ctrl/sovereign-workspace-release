...
def refuse_incompatible(model_id: str, backend: str) -> None:
    ...
    if backend in {"opencode", "powershell", "none"} and model_id not in (None, "", "null"):
        # backends that take no model still tolerate a null ref; a model_id of a runtime is forbidden
        pass
    ...
    if backend == "local_model":
        ...
        if hit and hit.get("locality") == "frontier":
            raise IncompatibleSelection(
                f"refused: frontier model {model_id!r} is incompatible with backend local_model")
    if backend == "powershell" and model_id:
        raise IncompatibleSelection(
            "refused: backend powershell takes a null model reference")
