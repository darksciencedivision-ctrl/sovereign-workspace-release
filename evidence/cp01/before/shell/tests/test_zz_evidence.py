...
class TestBuildManifest(unittest.TestCase):
    """§7.3 item 5 / A-5: shell/BUILD-MANIFEST.txt, generated by the test command."""

    def test_generate_build_manifest(self):
        root = os.path.join(WORKSPACE, "shell")
        entries = []
        for dirpath, dirnames, filenames in os.walk(root):
            dirnames[:] = [d for d in dirnames if d != "__pycache__"]
            for name in sorted(filenames):
                if name == "BUILD-MANIFEST.txt":
                    continue  # cannot hash a file whose content depends on this hash
                full = os.path.join(dirpath, name)
                rel = os.path.relpath(full, root).replace("\\", "/")
                entries.append((rel, sha256_file(full)))
        entries.sort()

        out = os.path.join(root, "BUILD-MANIFEST.txt")
        with open(out, "w", encoding="utf-8", newline="\n") as f:
            f.write("# SWS-BUILD-MANIFEST v1\n")
            f.write("# utc: {}\n".format(utc_now()))
            f.write("# producer: claude-code REM-01\n")
            f.write("# root: shell/\n")
            f.write("# excluded: __pycache__/**, BUILD-MANIFEST.txt (self)\n")
            f.write("# entries: {}\n".format(len(entries)))
            for rel, digest in entries:
                f.write("{}  {}\n".format(digest, rel))

        self.assertTrue(os.path.isfile(out))
        self.assertGreater(len(entries), 20, "manifest looks too small")
        # §7.3 item 5: the header's source revision is the first 12 hex of this file's hash.
        self.assertEqual(len(sha256_file(out)), 64)
