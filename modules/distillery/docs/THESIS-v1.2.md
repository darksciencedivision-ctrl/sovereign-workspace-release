...
**Thesis v1.2 — Post-Audit Remediation Candidate**
...
| **Status** | PR #1 merged; post-audit remediation and genuine live operator-mark completion verified; HG-0 PASS; HG-3 blocked on hardware capacity |
...
| **PR #1 merge commit** | `39aa889078b1754abf645e5ea75a3388656f8373` |
| **Post-merge evidence baseline** | `44f69077f0d674bd7bef5730c2fe41a685e64b03` |
| **Post-remediation implementation** | `55969b748946316fc25a63f5ebd69fd12f738012` |
...
The previously planned 32 GiB gfx906 trainer could not be located through bounded authorized discovery and is retired as an unverified planning target. Its assumed torch 2.7 / ROCm 6.3 stack, including `TORCH_BLAS_PREFER_HIPBLASLT=0` and `HSA_OVERRIDE_GFX_VERSION=9.0.6`, is retained only as `HISTORICAL_GFX906_PLANNED_STACK`; it is not the universal Grounded training environment.

The currently measured development workstation contains an 8 GiB-class NVIDIA RTX 5060 Ti. It is registered as `GND-DEV-EXEC-01` for non-gating engineering and is not treated as the canonical 4B/8B FP16 training node. Nominal static FP16 weights alone require approximately 7.45 GiB for 4B and 14.90 GiB for 8B, before activations, allocator overhead, adapters, gradients, optimizer state, temporary buffers, attention workspace, or sequence-length growth.

Grounded now specifies `GND-TRAINER-PRIMARY` by measured capability until physical hardware is designated:

```text
provisional minimum usable VRAM for bounded HG-3 smoke: 24 GiB
preferred trainer VRAM:                              >=32 GiB
vendor / GPU architecture:                          unassigned
backend stack:                                      selected after hardware
```

The 24 GiB minimum is a qualification threshold for bounded architecture smokes, not a guarantee of every sequence length. The preference for at least 32 GiB preserves sequence headroom, batch flexibility, stable 8B FP16 operation, retraining cadence, and lower OOM pressure.

The future canonical software stack follows the designated hardware: an NVIDIA trainer receives a pinned CUDA-qualified stack and an AMD trainer a pinned ROCm-qualified stack. The architecture remains backend-portable where practical. FP16 LoRA remains the canonical HG-3 path; QLoRA remains a separately labeled experiment and cannot prove the FP16 lane. See [D-HW-01](decisions/D-HW-01-grounded-primary-trainer-reality-rebaseline.md).
...
The v1.2 default decision contract is:
...
Confidence-tail policy A is explicit:

```text
two-sided 95% confidence interval
LCB = 97.5% one-sided lower-confidence bound
```

The paired gate and power analysis use this same tail policy. A true
one-sided 95% lower bound is available only when explicitly requested and
must not be confused with the default.

M/T/D, their authority, and their declaration timestamp must be recorded
before the evaluation run starts. The gate rejects missing, unparsable,
timezone-naive, equal, or later timestamps and stores the run identity, suite
identity/hash, and both timestamps in its evidence package.
...
Current state: **PASS**. The earlier imported G0-A/B labels remain classified as
import-time evidence, but they have now been superseded for gate purposes by a new live
QUICK success followed by an operator `/success` and a separate live DEEP controlled
cancellation followed by an operator `/fail`. Both marks use
`label_origin = live_operator_mark`; only hashes, timestamps, route/model metadata, and
validator state enter Git. G0-C recovery provenance and G0-D fail-closed admission
evidence remain sufficient. The NO_TRACE acceptance fixture is explicitly marked
`fixture = true`. HG-0 PASS does not authorize training, promotion, or deployment.
...
Current state: software foundation substantially implemented. HG-3 is blocked because no designated machine satisfies the primary trainer capability contract; HG-4 human pair audit remains blocked separately.
...
Designate and qualify `GND-TRAINER-PRIMARY`, then measure the pinned 4B and 8B candidates on that real trainer.
...
branch: main
PR #1 merge: 39aa889078b1754abf645e5ea75a3388656f8373
post-merge evidence baseline: 44f69077f0d674bd7bef5730c2fe41a685e64b03
post-remediation implementation: 55969b748946316fc25a63f5ebd69fd12f738012
...
- integration milestone suite: **27/27 passed**;
- original RC3 suite: **23/23 passed** at its preserved provenance commit;
- the independent integration baseline was verified on Linux with Python 3.11;
- the HG-0-complete suite is **42 passed with 133 subtests** on Windows/Python
  3.14, Windows/Python 3.11, and Linux/Python 3.12;
- the expanded post-remediation suite and schema/link/hygiene results are recorded in `runs/post-audit-remediation/`;
...
- actual Sovereign canon has been mapped and selectively imported from the non-Git source directory with original content hashes; the two-sided treaty contract passes;
- G0-A/B now PASS on new genuine live operator-mark evidence; G0-C/D remain PASS on preserved content-free evidence;
- margin predeclaration ordering, durable router terminal-state handling, explicit confidence tails, label-origin evidence, fixture alerts, and dirty-runtime checksum provenance are enforced;
...
After post-audit remediation validation:
...
CONTINUE FAIL-CLOSED SOURCE RESEARCH
       ↓
DESIGNATE + QUALIFY GND-TRAINER-PRIMARY
       ↓
HG-3 real 4B/8B FP16 architecture smoke
       ↓
HG-4 E4-pre audit
       ↓
G2 evaluation freeze
...
HG-3 remains unexecuted and blocked on hardware capacity. It resumes only after a real
machine is designated, measured, and backend-qualified against the primary trainer
capability contract, followed by separate compute authorization. G2–G6 remain
intentionally blocked until their prerequisites are satisfied. GR-8 arbitrary lineage-graph
acceptance and GR-9 historical-best-vector bridging are tracked for G2; the
vector must never be assembled from per-item maxima across runs or
checkpoints.
...
That is the operating philosophy of Grounded Distillery v1.2:
...
This v1.2 thesis preserves the frozen v1.1 record and adds:

- PR #1's merged canonical state and preserved RC3 lineage;
- the mapped import of actual Sovereign design, doctrine, evidence, registry,
  hardware, and characterization canon;
- a two-sided treaty contract covering Grounded, Sovereign, and shared
  infrastructure invariants;
- cryptographically distinguishable M/T/D predeclaration evidence;
- durable router writes and classified `ROLLBACK_FAILED` /
  `PROMOTION_WRITE_FAILED` terminal states;
- explicit two-sided 95% / one-sided 97.5% lower-bound semantics, shared by
  gate and power analysis;
- content-free G0 event hashes, explicit label origins, and the subsequent genuine live
  `/success` and `/fail` evidence that closes HG-0;
- provenance-complete checksums for the dirty external runtime;
- a deliberate LF repository policy and Windows/Linux verification;
- D-9's unchanged `UNKNOWN`, fail-closed state; and
- the D-HW-01 hardware reality rebaseline and HG-3's `BLOCKED_HARDWARE_CAPACITY` state.
