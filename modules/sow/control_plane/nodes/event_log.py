...
fact (`verify_file`). What it GUARANTEES: tampering with, removing or reordering rows of
the existing history breaks the seq/chain/hash checks.
Known limits (spec-audit F8; W-70/U512): verify_file cannot detect (1) TAIL TRUNCATION -
a chain cut clean at a row boundary stays valid - and (2) WHOLESALE REWRITE: GENESIS is
public and the chain deterministic, so a fabricated history with hashes recomputed from
GENESIS verifies clean (measured at HEAD cada0d3). Compensating controls: cross-checks
(the soak's spawn/exit pairing) for (1); an EXTERNAL HEAD ANCHOR for both - which does
not exist, is a NEW MECHANISM (PARK-R10 class), and stays a registered open debt (U512)
rather than being home-rolled here. There is no update or delete API by construction.
Appends are serialized under a lock and fsynced before returning, so an acknowledged
