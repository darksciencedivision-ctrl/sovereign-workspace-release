...
class NodeRegistry:
    ...
    def _paused_from_from_log(self, node_id: str, incarnation: int):
        """W-71: the recorded interrupted state, read back from this node's own durable
        transition rows (the last ``to=PAUSED`` row carries its ``frm``). No second store -
        the log IS the record. Returns None when no pause row exists or the log cannot be
        trusted, and the caller refuses: a resume nobody can ground in the record is refused
        rather than guessed."""
        found = None
        try:
            with Path(self._log.path).open("r", encoding="utf-8") as fh:
                for line in fh:
                    ...
                    if (row.get("kind") == "transition" and row.get("node_id") == node_id
                            and row.get("incarnation") == incarnation
                            and isinstance(payload, dict)
                            and payload.get("to") == NodeState.PAUSED.value):
                        found = payload.get("frm")
        except OSError:
            return None
        try:
            return NodeState(found) if found is not None else None
        except ValueError:
            return None
    ...
    def transition(self, node_id: str, requested: NodeState, *, incarnation: int | None = None, reason: str = "", pid: int | None = None) -> NodeRecord:
        with self._lock:
            ...
            if current is NodeState.PAUSED and requested is not NodeState.TERMINATED and requested is not NodeState.DISCONNECTED:
                if record.paused_from is None:
                    # W-71: paused_from lived only in memory, so a rehydrated PAUSED node
                    # could resume to any table-legal state. The interrupted state is already
                    # durable - every to=PAUSED transition row carries frm - so reconstruct it
                    # from this node's own rows instead of trusting memory that died.
                    record.paused_from = self._paused_from_from_log(record.node_id, record.incarnation)
                if record.paused_from is None or requested is not record.paused_from:
