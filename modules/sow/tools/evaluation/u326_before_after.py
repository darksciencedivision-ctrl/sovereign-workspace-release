...
Run from anywhere:  py -3.12 tools/evaluation/u326_before_after.py [base-commit]

W-75: importing this module used to EXECUTE the whole evaluation - argv read, a cwd-rooted
sys.path insert, product imports, a `git show` subprocess, a %TEMP% write and an unconditional
main() all sat at module top level, so a bare import from the wrong directory either ran git or
died on ModuleNotFoundError, and no importer was safe. Everything executable now lives under
main(); the repo root comes from __file__, never from the caller's cwd.
"""
...
def _base_from_argv() -> str:
    return sys.argv[1] if len(sys.argv) > 1 else "b529314"


def _load_before_module(base: str):
    """Fetch the pinned historical service and exec it beside the current one."""
    ...
    tmp = pathlib.Path(tempfile.mkdtemp()) / "before_collaboration_service.py"
    tmp.write_text(before_src, encoding="utf-8", newline="\n")
    spec = importlib.util.spec_from_file_location("before_collab", tmp)
...
def main() -> None:
    ...
    before = _load_before_module(base)
