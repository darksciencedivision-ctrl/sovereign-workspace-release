...
# Exit codes reserved by tools/run_phase19_pytest.py for its two breach flavours (U480).
SUITE_CEILING_CLEAN_EXIT = 124     # ceiling fired, zero test failures observed
SUITE_CEILING_FAILURE_EXIT = 123   # ceiling fired, failures observed -> stays FAIL


def _is_suite_wrapper_step(step: "Step") -> bool:
    """True only for steps whose command invokes the Phase 19 pytest wrapper."""
    return any("run_phase19_pytest" in str(part) for part in step.argv)
...
def run_step(step: Step) -> Result:
    ...
    if _is_suite_wrapper_step(step):
        # U480: a budget breach and a suite failure are different outcomes, carried here
        # through the wrapper's exit codes. Host load is NOT a product verdict.
        if proc.returncode == SUITE_CEILING_CLEAN_EXIT:
            return Result(
                step.name, NOT_VERIFIED,
                "the committed suite ceiling fired with NO test failure observed before "
                f"the kill - host load, not a product verdict. Tail: {tail}",
                proc.returncode, duration)
        if proc.returncode == SUITE_CEILING_FAILURE_EXIT:
            return Result(
                step.name, FAIL,
                "the committed suite ceiling fired with at least one test failure "
                f"OBSERVED before the kill - a breach carrying failures stays FAIL. Tail: {tail}",
                proc.returncode, duration)
    if step.debt:
