...
| TB-4 | Node ↔ harness/model | Harness config (`AGENTS.md`, MCP config, model output) is node-controlled untrusted input; enforcement lives outside the harness (supervisor + broker + workspace isolation) | **IMPLEMENTED** — supervisor + workspace isolation real (`node_runtime/supervisor/`, worktree isolation); harness-config-injection adversarial tests `tests/security/test_t2_harness_config_injection.py` (see T2) |
...
| T2 | Harness config injection (`AGENTS.md`, MCP config) | TB-4 | Node-controlled untrusted input; Sovereign policy never read from harness config; adversarial tests | **IMPLEMENTED** — policy-from-harness-config refusal real (policy loads from repo `control_plane/` only); adversarial tests `tests/security/test_t2_harness_config_injection.py` (4/4 py -3.12; host 3.14 skips the jsonschema-gated profile-loader leg) |
