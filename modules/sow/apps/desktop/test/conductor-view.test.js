...
test("F-37: an unknown greeting budget delivers without constructing the runtime journal",async()=>{
  ...
  assert.equal(r.written,true);assert.equal(sourceCalls,0);assert.equal(writes,1);
  assert.equal(r.context.state,"budget_unmeasurable");
});
