...
def provider_allowance_cap(provider: str) -> int:
    ...
    An id with NO recorded per-provider cap answers 0 at the authority and 0 here - the
    documented fail-closed absence semantics (live_authorization); a new provider cannot
    inherit another's concurrency by omission. W-65: the falsy branch used to read a recorded
    OR absent 0 as false and hand back the GLOBAL cap of 2 - this sentence was the false half
    of two contradictory docstrings, corrected here rather than in the authority, which told
    the truth."""
    recorded = provider_terminal_cap(provider)
    return min(MAX_ALLOWANCE, recorded)
