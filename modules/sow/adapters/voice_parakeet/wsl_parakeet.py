...
def silence_only_confidence(text: str) -> float:
    """NOT recognition confidence - the name IS the retraction (W-80 / [[U140]]).

    NeMo emits a log-probability, and calibrating that into a 0..1 score is an operator-ruled
    PARK-R10 boundary this loop refuses to cross, so this seam derives only what it honestly can:
    1.0 when the transcript is non-empty (the engine heard SOMETHING) and 0.0 when it heard
    nothing. The CLARIFY gate fed from this value therefore protects against SILENCE alone;
    mishearing passes at 1.0 by construction. A JSON confidence already in [0, 1] bypasses this
    derivation entirely (test seam).

    T9 note: docs/THREAT_MODEL.md's T9 claim on this surface transferred to W-83 per card W-80.
    """
...
def parse_transcript(stdout: str) -> Transcript:
    ...
    would route EVERY real utterance to CLARIFY (voice "unusable" again). W-80 RETRACTION: what
    this seam derives is NOT recognition confidence - NeMo's log-probability is never calibrated
    here (operator PARK-R10 boundary, U140). `silence_only_confidence` maps a non-empty transcript
    to 1.0 and an empty one (the engine heard nothing) to 0.0, so the CLARIFY gate protects against
    SILENCE alone; mishearing passes at 1.0 by construction. A JSON `confidence` already in [0, 1]
    is respected (test seam); an out-of-range one (a raw log score) is ignored in favour of the
    text-derived value."""
    for line in reversed([ln for ln in (stdout or "").splitlines() if ln.strip()]):
        ...
        if isinstance(obj, dict) and isinstance(obj.get("text"), str):
            ...
            if isinstance(conf, (int, float)) and 0.0 <= float(conf) <= 1.0:
                ...
                confidence = silence_only_confidence(text)
