"""SW-18: graceful shutdown on the shell's SWS_SHUTDOWN_EVENT for the SOVEREIGN product server.

The workspace shell launches this module inside a Windows Job Object with CREATE_NO_WINDOW, so a
...
lets the module stop serving and flush its state on its own terms. This mirrors the canonical
shell/src/graceful.py contract (a module cannot import the shell package); the retained suite
drives the real entry point under the real Job supervisor to prove it
(tests/remediation/test_sw18b_graceful_owned.py).
"""
