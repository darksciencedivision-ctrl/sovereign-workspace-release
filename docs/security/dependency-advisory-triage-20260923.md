...
finding is unexplained and must be triaged before release. **After the 2026-09-24 resolution
(section 6) the anyio advisories no longer apply to either lock, so a repeat scan should surface
only the chromadb advisories.**

## 6. Resolution (2026-09-24) - anyio closed

Operator approved recommended actions 1 and 2 (directive section 5.0).

- **Debate:** `modules/debate/requirements.lock.txt` now pins `anyio==4.15.1` (matching sovereign).
  The `--hash=` line was regenerated by the F-071 lock tool (`hash_python_locks.write_lock`, which
  records the sha256 of the published win_amd64/any wheels); the hash equals the wheel hash already
  in the sovereign lock. No other Debate pin changed. anyio 4.15.1 adds
  `typing_extensions>=4.16.0` (Python < 3.15); the lock already pins `typing_extensions==4.16.0`.
  `modules/debate/MANIFEST-SHA256.json` carries the new lock digest. The Debate venv was
  re-provisioned with `pip install --require-hashes --only-binary=:all: --no-deps -r
  requirements.lock.txt`; `pip check` is clean.
- **Sovereign:** the venv was re-provisioned from its lock the same way. That reconciled the anyio
  drift (installed 4.14.2 -> 4.15.1) and 21 other installed-vs-lock drifts that had accumulated
  (all patch/minor upgrades to the locked versions); `pip check` is clean.
- **Re-scan (OSV `/v1/query`, 2026-09-24):** `anyio 4.15.1` -> no advisories;
  `anyio 4.14.1` -> GHSA-3w57-8xmc-8v26, GHSA-5p39-cfhj-2xmp, GHSA-82r6-8w77-94w6 (the three
  closed here).
- **Note:** `modules/debate/snapshot/ENVIRONMENT.json` is the historical v1.2 baseline capture
  attributed to commit d03a1b7 and still records 4.14.1; it is a dated record, not a pin, and was
  deliberately not rewritten.
- **Guard:** `test_sw23_dependency_boundaries.py::test_debate_anyio_lock_meets_fix_floor` now
  requires the Debate lock at or above 4.14.2 with a real wheel hash (the pending-bump allowance
  is removed).
