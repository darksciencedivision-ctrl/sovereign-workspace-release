# Start-OpenCode-CP-M1.ps1 - operator tooling only. Not part of the shell; not a module launch.
# Launches OX ALPHA (OpenCode) at the Production Workspace root for the merged CP-01 + CP-02 loop (goals G1-G124).
#
# One flattened ladder, G1-G124, gates 8a-8j then 9a-9j. The merge is safe because there is
# exactly ONE writer. This launcher proves that: it refuses to start if the superseded CP-01
# session still has a live writer, and it verifies that the protected-root evidence CP-M1 G4
# ADOPTS is actually on disk before handing over.
...
$required = @(
    ...
    'docs\OX-ALPHA-DIRECTIVE-CP-M1.md',
    'docs\OX-ALPHA-CP-M1-PASTE.md',
...
foreach ($f in @('docs\SWS-UI-001-v1.2-ADDENDUM-03.md','docs\OX-ALPHA-DIRECTIVE-CP-M1.md')) {
...
Write-Host ""
Write-Host "-- adopted CP-01 Band 0 evidence (ADD-03 §2 / CP-M1 G4) --"
$adopted = @(
...
if ($listening) {
    ...
    if ($only8765) {
        Write-Warning "Only 8765 is held - that is the operator's Token Center. ADD-03 A-3 authorizes the builder to stop it by pid immediately before Band 9 (G50). Proceeding."
    } else {
...
if (Test-Path -LiteralPath $liveOp) {
    ...
    if ($lo.live_operation_authorized -eq $true) {
        ...
        Write-Warning "  The run will STOP at G2 with PROVIDER_SPEND_CONTRADICTION until you reconcile this."
...
foreach ($t in @('D:\multi model terminal app\sovereign-orchestration-workspace','D:\Sovereign Distillery','D:\Sov 1','D:\Token Piggy Bank')) {
    ...
    if (Test-Path -LiteralPath (Join-Path $t '.git')) {
        ...
        if ($h1 -ne $h2) { Write-Warning "$t HEAD moved - the run will stop at G4" } else { Write-Host ("quiescent: {0}  {1}" -f $t, $h1) }
...
Set-Clipboard -Value (Get-Content -LiteralPath (Join-Path $Workspace 'docs\OX-ALPHA-CP-M1-PASTE.md') -Raw)
Write-Host ""
Write-Host "CP-M1 kickoff copied to clipboard. Paste with Ctrl+V as the FIRST message of the session."
Write-Host "One ladder: G1-G124. Twenty gates: 8a-8j then 9a-9j."
Write-Host "Order is the safety argument - G(n) waits on G(n-1). CP-M1 is self-contained; the older directives are provenance only."
